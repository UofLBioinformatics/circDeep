...
def build_ACNN_BLSTM_model(k, s, vector_dim, root_path, MAX_LEN,pos_sequences, neg_sequences,seq_file):
    model1 = gensim.models.Word2Vec.load(root_path + 'word2vec_model' + '_' + str(k) + '_' + str(s) + '_' + str(vector_dim))
    ...
    checkpointer = ModelCheckpoint(
        filepath=str(k) + ' ' + str(s) + ' ' + str(vector_dim) + str(MAX_LEN) + 'bestmodel_deepsea.hdf5', verbose=1,
        save_best_only=True)
    earlystopper = EarlyStopping(monitor='val_loss', patience=6, verbose=1)

    print('Training model...')
    history = model.fit(X_train, y_train, nb_epoch=90, batch_size=128, shuffle=True,
...
def extract_ACNN_BLSTM(k, s, vector_dim, root_path, MAX_LEN,testing_sequences,seq_file):
    model1 = gensim.models.Word2Vec.load(root_path + 'word2vec_model' + '_' + str(k) + '_' + str(s) + '_' + str(vector_dim))

    seqs=seq2ngram(testing_sequences, k, s, 'seq_'+str(k)+'_'+str(s)+'.txt',model1.wv)

    X = pad_sequences(seqs, maxlen=MAX_LEN)
    model=load_model(str(k) + ' ' + str(s) + ' ' + str(vector_dim) + str(MAX_LEN) + 'bestmodel_deepsea.hdf5')
    intermediate_layer_model = Model(inputs=model.input,
                                     outputs=model.get_layer('myfeatures').output)
...
def bed_to_fasta(bed_file,fasta_file,sequence_file,genome_fasta):
    ...
    dirname = os.path.dirname(sequence_file)
    if not os.path.exists(dirname):
        os.makedirs(dirname)
...
def concatenate_rcm_files(rcm_file,data_dir):
    ...
    with open(rcm_file, 'w') as outfile:
        outfile.write(lines['header'] + 'posneg' + '\n')
...
def extract_features_testing(testing_bed,genome,bigwig,gtf,data_dir):
    ...
    testing_conservation_feature_file=pos_data_bed+'.cons'

    bed_to_fasta(testing_bed,testing_fasta ,testing_sequences, genome)

    extract_feature_conservation_CCF(testing_bed, bigwig, gtf, testing_conservation_feature_file)

    extract_rcm_features_testing(testing_fasta, genome,rcm_file,data_dir)
    extract_ACNN_BLSTM(3, 1, 40, data_dir,8000,testing_sequences,seq_file)

    return testing_fasta,testing_sequences,cons_file,rcm_file,seq_file

def extract_features_training(pos_data_bed,neg_data_bed,genome,bigwig,gtf,data_dir):
    cons_file=data_dir+'conservation_features.txt'
    rcm_file=data_dir+'rcm_features.txt'
    seq_file=data_dir+'seq_features.txt'
    ...
    word2vect(3, 1, 40, data_dir, pos_sequences)
    build_ACNN_BLSTM_model(3, 1, 40, data_dir,8000,pos_sequences, neg_sequences,seq_file)
...
def get_rnn_fea(train, sec_num_hidden = 128, num_hidden = 128):
    ...
    return model	
def train_circDeep(data_dir, model_dir, genome, bigwig, gtf,positive_bed,negative_bed,seq=True, rcm=True,  cons = True):
    pos_data_fasta, pos_sequences, neg_data_fasta, neg_sequences, cons_file, rcm_file, seq_file=extract_features_training(positive_bed, negative_bed, genome, bigwig, gtf, data_dir)
    training_data = load_data(data_dir, seq, rcm,  cons,cons_file, rcm_file, seq_file)
    ...
    model.fit(training, y, batch_size=128, nb_epoch=40, verbose=1, validation_data=(validation, val_y), callbacks=[earlystopper])

    model.save(os.path.join(model_dir,'bestmodel.pkl'))
...
def test_circDeep(data_dir, model_dir, genome, bigwig, gtf,testing_bed,seq=True, rcm=True,  cons = True,outfile='prediction.txt'):
    testing_fasta, testing_sequences, cons_file, rcm_file, seq_file=extract_features_testing(testing_bed, genome, bigwig, gtf, data_dir)
    test_data = load_data(data_dir, seq, rcm,  cons,cons_file, rcm_file, seq_file)
    ...
    model = load_model(os.path.join(model_dir, 'bestmodel.pkl'))
...
def run_circDeep(parser):
    ...
    if train:
        ...
        train_circDeep(data_dir, model_dir,genome, bigwig, gtf,positive_bed,negative_bed,  seq = seq, rcm=rcm,cons=cons)

    else:

        print ('model prediction')

        test_circDeep(data_dir, model_dir, genome, bigwig, gtf,testing_bed,seq=True, rcm=True,  cons = True,outfile='prediction.txt')


def parse_arguments(parser):

    parser.add_argument('--data_dir', type=str,default='features/', metavar='<data_directory>', help='Under this directory, you should have feature file: sequences.fa.gz ')
    ...
    parser.add_argument('--predict', type=bool, default=False,  help='Predicting circular RNAs. if using train, then it will be False')
    ...
    parser.add_argument('--genome', type=str, default='hg38.fasta',help='The Fasta file of genome')

    parser.add_argument('--gtf', type=str, default='hg38.gtf',help='The gtf annotation file. e.g., hg38.gtf')

    parser.add_argument('--bigwig', type=str, default='hg19.100way.phyloP100way.bw',help='conservation scores in bigWig file format')

    parser.add_argument('--positive_bed', type=str, default='circular_rna.bed', help='BED input file for circular RNAs for training, it should be like:chromosome    start    end    gene')

    parser.add_argument('--negative_bed', type=str, default='other_long_non_coding_rna.bed',help='BED input file for other long non coding RNAs for training, it should be like:chromosome    start    end    gene')
    parser.add_argument('--testing_bed', type=str, default='test.bed',help='BED input file for testing data, it should be like:chromosome    start    end    gene')
