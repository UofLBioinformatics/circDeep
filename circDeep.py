...
def build_ACNN_BLSTM_model(k, s, vector_dim, root_path, MAX_LEN, pos_sequences, neg_sequences, seq_file, class_file):
    model1 = gensim.models.Word2Vec.load(
        root_path + 'word2vec_model' + '_' + str(k) + '_' + str(s) + '_' + str(vector_dim))
    ...
    checkpointer = ModelCheckpoint(
        filepath='bestmodel_ACNN_BLSTM_' + str(k) + ' ' + str(s) + ' ' + str(vector_dim) + str(MAX_LEN) + '.hdf5',
        verbose=1,
        save_best_only=True)
...
def extract_ACNN_BLSTM(k, s, vector_dim, root_path, MAX_LEN, testing_sequences, seq_file):
    ...
    X = pad_sequences(seqs, maxlen=MAX_LEN)
    model = load_model('bestmodel_ACNN_BLSTM_' + str(k) + ' ' + str(s) + ' ' + str(vector_dim) + str(MAX_LEN) + '.hdf5')
    intermediate_layer_model = Model(inputs=model.input,
                                     outputs=model.get_layer('myfeatures').output)
...
def extract_features_testing(testing_bed, genome, bigwig, gtf, data_dir):
    ...
    testing_conservation_feature_file = testing_bed + '.cons'
...
def extract_features_training(pos_data_bed, neg_data_bed, genome, bigwig, gtf, data_dir):
    ...
    print('11111')
    build_ACNN_BLSTM_model(3, 1, 40, data_dir, 8000, pos_sequences, neg_sequences, seq_file, class_file)
    print('22222')
...
def train_circDeep(data_dir, model_dir, genome, bigwig, gtf, positive_bed, negative_bed, seq=True, rcm=True, cons=True):
    pos_data_fasta, pos_sequences, neg_data_fasta, neg_sequences, cons_file, rcm_file, seq_file = extract_features_training(
        positive_bed, negative_bed, genome, bigwig, gtf, data_dir)
    ...
    model.fit(training, y, batch_size=128, nb_epoch=40, verbose=1, validation_data=(validation, val_y),
...
def run_circDeep(parser):
    ...
    if train:
        ...
        model=train_circDeep(data_dir, model_dir, genome, bigwig, gtf, positive_bed, negative_bed, seq=seq, rcm=rcm,
                       cons=cons)
...
def parse_arguments(parser):
    ...
    parser.add_argument('--positive_bed', type=str, default='data/pos.bed',
                        help='BED input file for circular RNAs for training, it should be like:chromosome    start    end    gene')

    parser.add_argument('--negative_bed', type=str, default='data/neg.bed',
